...
--zad2
UPDATE obiekty SET geom = (SELECT ST_MakePolygon( ST_AddPoint(foo.geom, ST_StartPoint(foo.geom)) )
FROM (
  SELECT geom  FROM obiekty WHERE  gid = '4' ) As foo) WHERE gid = '4';
SELECT * FROM obiekty WHERE gid = '4';

--zad3

INSERT INTO obiekty 
VALUES (7,'obiekt7',(SELECT ST_Collect((SELECT geom FROM obiekty WHERE gid = '3'),(SELECT geom FROM obiekty WHERE gid = '4'))));
